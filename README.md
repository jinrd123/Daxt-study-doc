...
因为搜索引擎对于js文件的不识别，所以导致csr的网站seo差，就需要ssr。如果要提升一个网站的seo排名，抓住三个方面：文字、多媒体(图片...)、链接。（其实这三者就把网站的组成元素概括全了，无论什么网站无非就是这三个东西）

链接分为内部链接和外部链接，内部链接即网站内的链接，链接到另一个网站，对于内部链接要尽可能的做到链接到的网站与本网站息息相关（链接到的网站与本站毫无关系就会降低seo排名）；外部链接即其他网站链接到我们的网站，外部链接要尽可能的多，就会有利于我们的网站的seo。

文字，文字内容要做到原创，百度谷歌等搜索引擎把文字是否原创视为很重要的一个指标。

还有多媒体，比如图片，图片的高清与远程与否也影响seo排名。

当然优化这三者的前提是爬虫能够识别我们网站里的内容，即ssr所做的事情。

当然普通的csr项目并不是说就完全不能获得好的seo排名，有一种预渲染技术，即我们启动一个服务，在网络爬虫访问我们的spa应用时会识别出来，预渲染服务类似于开启一个小浏览器，先访问我们的spa应用，获取到js加载完毕后的网页源代码，然后把这个源代码返回给爬虫。所以说如果不是对首屏有特别高的要求，只是要求一个seo排名的话，ssr也不是必选项。